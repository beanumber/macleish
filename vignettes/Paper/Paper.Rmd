...
```{r}
# Dominique: this installs packages from CRAN (which does not have the new
# data in it yet). We need to use the locally built version which does have it.
# i.e. rebuild the package and comment this line out
...
Two tree plot datasets is available through `tree_diameter1` and `tree_diameter2`. The datasets present change in tree diameter over
a set of years for two plots dominated by the parasites Hemlock Woolly Adelgid. 
...
For example, using the `tree_diameter1` dataset, the boxplot below demonstrates the distribution of diameter for different species of trees at different positions between 2010-2015. 
