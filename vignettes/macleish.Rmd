...
* WSpd_mps: Average windspeed (m/s)
* Wdir_deg: Wind direction (degree)
* RH_per_Avg: Relative Humidity (mm)
* Press_mb_Avg: Average atomspheric pressure (mb)
* Rain_mm_Tot: Main Rainfall amount per day (mm)
* SlrW_Avg: WhatelyMet Average Solar Radiance (W/m^2)
* PAR_Den_Avg: OrchardMet Photosynthetically Active Radiation (sunlight between 400 and 700 nm) in average density (W/m^2)
* PAR_Tot_Avg: OrchardMet Photosynthetically Active Radiation (sunlight between 400 and 700 nm) in average total over measurement period (W/m^2)

#### Climatic Indicators 
Climatic indicators are used in this doucumentation to describe climatic events, and here we list the definitions for these terms for you to better understand this documentation.  
...
Here are the definitions of Climatic Indicators:

* Freezing Degree Days — The number of degrees Celsius that the daily mean temperature is below 0 °C.
* Growing Degree Days — The number of degrees Celsius that the daily mean temperature exceeds 10 °C .
* Spring Onset Date — The first day of the year where the average high temperature of that day and the previous four days averages 10 °C and no subsequent five day period until mid-summer has an average high temperature below 6.1 °C. 
* Last Spring Freeze Date — The last day between January 1st and mid-summer with a low temperature below 0 °C. 
* First Autumn Freeze Date — The first day after mid-summer with a low temperature below 0 °C. 
* Length of the Growing Season — The number of days between the last spring freeze and first autumn freeze. 
* Frequency of 1” Precipitation Events — The number of days per year with precipitation of at least 25.4 mm. 
* Frequency of 2” Precipitation Events — The number of days per year with precipitation of at least 50.8 mm. 
* Frequency of 3” Precipitation Events — The number of days per year with precipitation of at least 76.2 mm. 
...
daily <- whately_2015 %>%
  ...
  summarize(N = n(), avg_temp = mean(Temp_C_Avg), 
            max_temp = max(Temp_C_Avg), min_temp =
              min(Temp_C_Avg))
```
...
The average daily temperature range recorded from both stations (daily maximum-minimum) was -10.5 °C. The maximum daily temperature range was 20.37 °C on May 24th at WhatelyMet. The minimum daily temperature range was 0.876 °C and occurred on January 1st at WhatelyMet. However, daily temperature ranges of less than three degrees also occurred on February 9th, April 9th, June 28th, and December 30th.

```{r humidity-table}
monthly_w <- whately_2015 %>%
  mutate(the_month = month(when, label = TRUE, abbr = FALSE)) %>%
  group_by(the_month) %>%
  summarize(avg_humidity_w = mean(RH_per_Avg))
monthly_o <- orchard_2015 %>%
  mutate(the_month = month(when, label = TRUE, abbr = FALSE)) %>%
  group_by(the_month) %>%
  summarize(avg_humidity_o = mean(RH_per_Avg))
monthly_w %>%
  inner_join(monthly_o, by = "the_month")
```
...
monthly_w <- whately_2015 %>%
  mutate(the_month = month(when, label = TRUE, abbr = FALSE)) %>%
  group_by(the_month) %>%
  summarize(total_precip_w = sum(Rain_mm_Tot))
monthly_o <- orchard_2015 %>%
  mutate(the_month = month(when, label = TRUE, abbr = FALSE)) %>%
  group_by(the_month) %>%
  summarize(total_precip_o = sum(Rain_mm_Tot))
monthly_w %>%
  inner_join(monthly_o, by = "the_month")
```
