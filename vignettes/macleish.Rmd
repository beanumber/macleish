...
The [`macleish`](http://www.github.com/beanumber/macleish) package for R contains two weather data collection at the Ada & Archibald MacLeish Field Station in Whately, MA, as well as spatial data detailing various features of the property. This vignette will illustrate how these data can be used. 
...
Two data frames are included in the package: `whately_2015` and `orchard_2015`. These contains weather data, averaged over every ten minute period for the calendar year 2015. The `head` and `tail` will give you the first (last) 6 readings from the whately weather station from 2015, respectively.
...
To access data during wider time spans, make use of the `etl` framework to download the data and import it into a database. Please see the documentation on `etl_extract` for further details and examples. 
...
ggplot(data = whately_2015, aes(x = when, y = Temp_C_Avg)) +
  ...
  geom_smooth() + 
  geom_line(data = daily, aes(x = as.POSIXct(the_date), y = avg_temp)) + 
  xlab(NULL) + ylab("Temperature (Celsius)")
```


The monthly average temperature in 2015 ranged from -6.4 °C in January to 21.6°C in July. 
...
Now, we can create a scatterplot to show how much rainfall you would have per day over the course of the whately_2015 weather data.
...
ggplot(daily2015, aes(x = day_of_year, y = cumsum(avgRain))) +
  geom_line()
```
