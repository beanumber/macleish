...
author: "Ben Baumer, Rose Goueth, Wencong Li and Weijia Zhang (Vega)"
date: "03-28-2016"
...
This R package can be used to process and analyze data from the [MacLeish weather station](http://www.smith.edu/ceeds/macleish.php). The following text will give you an outline of how to use the package.
...
##### Lowest Temperature
We can use the dplyr package to find some summary statistics.
```{r}
whately_2015 %>%
  arrange(Temp_C_Avg) %>%
  head()
```
As we can see, on Jan.8th the temperature at Whately dropped to -22.28 degrees celsius, which was the minimum of the year. However, that only appeared for once. Instead, on Feb.16th, the temperature reached the similar level for more than once.

That's not the same case for Orchard.
```{r}
orchard_2015 %>%
  arrange(Temp_C_Avg) %>%
  head()
```
The temperature reached the minimum of around -90 degrees celsuis (-130 degrees fahrenheit) on Nov.23rd
